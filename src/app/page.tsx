...
const REMOVAL_LABELS: Record<RemovalType, string> = {
  none: "No removal / 无卸甲 ",
  extension_removal: "Extension removal / 卸延长甲 ",
  natural_removal: "Natural nail removal / 卸本甲 ",
};

const STYLE_LABELS: Record<StyleType, string> = {
  basic: "Basic / 基础款 ",
  extension: "Extension / 延长款 ",
};
...
export default function Home() {//看起来是给左侧赋值，但本质上是在声明组件的多个 state。每个 useState 都在 React 内部占据一个固定顺序的位置，并指定这个 state 的初始值和类型；左侧通过数组解构拿到当前值和对应的 setter 函数。
  ...
  const [styleType, setStyleType] = useState<StyleType>("basic");
  ...
  const currentBookingDurationMin =
    BASE_BOOKING_DURATION_MIN +
  ...
  async function refreshSlots() {
    ...
    try {
      const res = await fetch(
        `/api/available?date=${date}&removalType=${encodeURIComponent(removalType)}&styleType=${encodeURIComponent(styleType)}`
      );
      if (!res.ok) {
...
const renderableSlots = useMemo(() => {
  ...
  const requiredSteps = Math.ceil(currentBookingDurationMin / STEP_MIN);
  ...
  return slotMinutes
    ...
    .map(toHHMM);
}, [slots, currentBookingDurationMin]);
  useEffect(() => {
    refreshSlots();
      ...
      <div className="mx-auto max-w-2xl space-y-4">
        ...
        <div className="rounded-xl border p-4 space-y-3">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium">Removal / 卸甲</label>
            <select
              value={removalType}
              onChange={(e) => setRemovalType(e.target.value as RemovalType)}
              className="rounded-md border px-3 py-2"
            >
              {Object.entries(REMOVAL_LABELS).map(([value, label]) => (
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium">Style / 本次款式</label>
            <select
              value={styleType}
              ...
              {selectedSlot && (
                <div className="mt-4 rounded-lg border p-3 space-y-2">
                  ...
                  <div className="text-sm text-gray-600">
                    Duration / 预计时长：{currentBookingDurationMin} min
                  </div>
                  <div className="text-sm text-gray-600">{REMOVAL_LABELS[removalType]}</div>
                  <div className="text-sm text-gray-600">{STYLE_LABELS[styleType]}</div>
